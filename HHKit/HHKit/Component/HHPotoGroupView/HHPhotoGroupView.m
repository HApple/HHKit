...
- (id)copyWithZone:(NSZone *)zone {
    HHPhotoGroupItem *item = [self.class new];
    return item;
}
@end
...
@interface HHPhotoGroupCell : UIScrollView <UIScrollViewDelegate>
...
@property (nonatomic, strong) HHPhotoGroupItem *item;
@property (nonatomic, readonly) BOOL itemDidLoad;
- (void)resizeSubviewSize;
@end

@implementation HHPhotoGroupCell
...
- (void)setItem:(HHPhotoGroupItem *)item {
...
@interface HHPhotoGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation HHPhotoGroupView
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((HHPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    HHPhotoGroupCell *cell = [self cellForPage:self.currentPage];
    HHPhotoGroupItem *item = _groupItems[self.currentPage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    HHPhotoGroupCell *cell = [self cellForPage:currentPage];
    HHPhotoGroupItem *item = _groupItems[currentPage];
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(HHPhotoGroupCell *cell, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            HHPhotoGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                HHPhotoGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (void)updateCellsForReuse {
    for (HHPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (HHPhotoGroupCell *)dequeueReusableCell {
    HHPhotoGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
...
- (HHPhotoGroupCell *)cellForPage:(NSInteger)page {
    for (HHPhotoGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    HHPhotoGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
